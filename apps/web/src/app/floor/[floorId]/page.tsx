...
export default async function FloorPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      {/* ヘッダー (桃色) */}
      <header className="bg-brand-900 text-green-400 flex-shrink-0 sticky top-0 z-30 shadow-md overflow-hidden relative">
        {/* ヘッダー背景の薄い花模様 */}
        ...
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center gap-3 relative">
          <Link href="/" className="flex items-center gap-1 text-green-400/70 hover:text-green-400 transition-colors">
          ...
          <div className="hidden sm:flex items-center gap-1">
            {[1, 2, 3, 4].filter((f) => f !== floor).map((f) => (
              <Link key={f} href={`/floor/${f}`}>
                <button className="text-xs text-green-400 hover:text-green-200 bg-pink-800/50 hover:bg-pink-700/60 border border-pink-600/40 hover:border-pink-400/60 rounded px-2 py-1 transition-colors font-medium">
                  {f}F
                </button>
              </Link>
            ))}
      ...
      {/* ピン数インジケーター (緑色) */}
      {pinnedCount > 0 && (
        <div className="bg-green-700 text-green-400 text-center text-sm py-2 px-4 flex items-center justify-center gap-2">
